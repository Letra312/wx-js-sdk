export as namespace wx;
   ...
   * @param setting 配置对象
   */
   export function config(setting: WxConfig): void;

  /**
   * 配置微信初始化成功后的回调
   * @param x 回调
   */
  export function ready(x: () => void): void;

  /**
   * 配置微信初始化失败后的回调
   * @param err 失败回调
   */
  export function error(err: (res: any) => void): void;
   ...
   */
  export function checkJsApi(setting: CheckApiConfig): void;
   ...
   */
  export function onMenuShareTimeline(setting: ShareTimelineConfig): void;
   ...
   */
  export function onMenuShareQQ(config: MenuShareQQ): void;
   ...
   */
  export function onMenuShareWeibo(config: MenuShareWeibo): void;

  /**
   * 获取“分享到QQ空间”按钮点击状态及自定义分享内容接口
   */
  export function onMenuShareQZone(config: MenuShareWeibo): void;
   ...
   * 拍照或从手机相册中选图接口
   */
  export function chooseImage(config: ChooseImageConfig): void;
   ...
   * 预览图片接口
   */
  export function previewImage(config: PreviewImageConfig): void;
   ...
   * 上传图片
   */
  export function uploadImage(config: UploadImageConfig): void;
  ...
  export function downloadImage(config: DownLoadImageConfig): void;
   ...
   */
  export function getLocalImgData(config: GetLocalImgDataConfig): void;
   ...
   * 停止录音
   */
  export function stopRecord(success: (res: {localId: string}) => void): void;

  /**
   * 录音时间超过一分钟没有停止的时候会执行回调
   */
  export function onVoiceRecordEnd(complete: (res: { localId: string }) => void): void;
   ...
   * 停止播放后的回调
   */
  export function onVoicePlayEnd(success: (res: { localId: string }) => void): void;
   ...
   */
  export function uploadVoice(config: UploadVoiceConfig): void;
  ...
  interface downloadVoiceConfig {
   ...
   export function downloadVoice(config: downloadVoiceConfig): void;

  /**
   * 识别音频并返回识别结果接口
   */
  export function translateVoice(config: UploadVoiceConfig): void;

  /**
   * 获取网络状态, var networkType = res.networkType; 返回网络类型 2g，3g，4g，wifi
   */
  export function getNetworkType(success: (res: { networkType: string }) => void): void;
   ...
   * 微信内置地图查看位置
   */
  export function openLocation(config: OpenLocationConfig): void;
   ...
   * 获取地理位置接口
   */
  export function getLocation(config: GetLocationConfig): void;
   ...
   * 开启查找周边 ibeacon 设备接口
   */
  export function startSearchBeacons(config: StartSearchBeaconsConfig): void;
   ...
   * 关闭查找完成后的回调函数
   */
  export function stopSearchBeacons(config: StopSearchBeaconsConfig): void;
   ...
   */
  export function onSearchBeacons(config: OnSearchBeaconsConfig): void;
   ...
   * 批量隐藏功能按钮接口
   */
  export function hideMenuItems(config: MenuItemsConfig): void;

  /**
   * 批量显示功能按钮接口
   */
  export function showMenuItems(config: MenuItemsConfig): void;
   ...
   * 调起微信扫一扫接口
   */
  export function scanQRCode(config: ScanQRCodeConfig): void;
   ...
   * 跳转微信商品页接口
   */
  export function openProductSpecificView(config: OpenProductSpecificViewConfig): void;
   ...
   * 拉取适用卡券列表并获取用户选择信息
   */
  export function chooseCard(config: ChooseCardConfig): void;
   ...
   * 批量添加卡券接口
   */
  export function addCard(config: AddCardConfig): void;
   ...
   * 查看微信卡包中的卡券接口
   */
  export function openCard(config: OpenCardConfig): void;
   ...
   * 发起一个微信支付请求
   */
  export function chooseWXPay(config: ChooseWXPayConfig): void;
